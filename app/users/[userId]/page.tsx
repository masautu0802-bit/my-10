...
async function getUserProfileData(userId: string, currentUserId?: string) {
...
export default async function UserProfilePage({
  ...
  const profileData = await getUserProfileData(userId, currentUser?.id);
  if (!profileData) notFound();

  const { user, followingUsersCount, followersCount, followingShopsCount, totalShopFollowers, ownedShops, isFollowing } = profileData;
  ...
  return (
    <div className="bg-bgwarm min-h-screen flex flex-col antialiased max-w-md mx-auto shadow-2xl">
      ...
      <main className="flex-1 pb-24 px-5">
        ...
        {/* 所有ショップ一覧 */}
        <div className="py-6">
          <h2 className="text-lg font-bold text-text-main mb-4">ショップ</h2>
          ...
          {ownedShops.length === 0 ? (
            <div className="text-center py-12 text-text-main/60">
              ...
              <p className="text-sm font-medium">まだショップがありません</p>
            </div>
          ) : (
